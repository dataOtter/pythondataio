...
def append_row(filename, row):
    ...
    # check that the row has the same length as those in the file/as there are columns

    csv_ndarray = get_data(filename)
    if len(csv_ndarray[0]) != len(row):
        raise Exception("This row has more entries than there are columns.")
...
def save_data(filename, ndarray):
    ...
    with open(filename, "w", newline="\n") as csv_file:
        ...
        for row in ndarray:
            length = len(row)
            if length != columns:
                raise Exception("These rows do not have the same number of entries as there are columns!")
            writer.writerow(row)
